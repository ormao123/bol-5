...
function Combo()
	if Config.combo.comboactive then
		...
		if Config.combo.usee and Eready then
			local CastPosition,  HitChance,  Position = VP:GetCircularCastPosition(ts.target, 0.8, 200, 900)
		...
		if ts.target ~= nil then
			if Config.combo.useq and Qready and GetDistance(ts.target) < 1200 then
				if Config.combo.usew and Wready and GetDistance(ts.target, myHero) < 1200 then
					CastSpell(_W)
				end
				if Ehit(ts.target) then
					local CastPosition, HitChance, Position = VP:GetCircularCastPosition(ts.target, 0.5, 75, 1200)
					if CastPosition and HitChance >= 2 and GetDistance(CastPosition) < 1200 and ts.target.dead == false then
							CastSpell(_Q, CastPosition.x, CastPosition.z)
					end
				else
					local CastPosition,  HitChance,  Position = VP:GetLineCastPosition(ts.target, 0.5, 75, 1100, 1500, myHero, true)
